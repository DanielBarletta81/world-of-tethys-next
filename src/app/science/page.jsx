...
export default function FieldStationPage() {
  ...
  return (
    <main className="min-h-screen bg-[#080a0c] text-cyan-50 font-sans selection:bg-cyan-900 selection:text-white pb-24">
      ...
      <header className="pt-24 pb-12 px-6 border-b border-cyan-900/30 bg-[#050607]">
        <div className="max-w-7xl mx-auto text-center">
          <p className="text-xs font-mono text-cyan-600 uppercase tracking-[0.3em] mb-2">
            Sector 4: Research & Analysis
          </p>
          <h1 className="text-4xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-b from-cyan-100 to-cyan-800 uppercase tracking-tighter mb-6">
            Field Station Alpha
          </h1>
          <TriFoldNav />
        </div>
      ...
      <div className="sticky top-0 z-40 bg-[#080a0c]/95 backdrop-blur border-b border-cyan-900/30 mb-12">
        <div className="max-w-4xl mx-auto flex justify-center gap-2 p-2 overflow-x-auto hide-scrollbar">
          <TabButton 
            id="telemetry" 
      ...
      <div className="max-w-7xl mx-auto px-6 min-h-[60vh]">
        <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 border-t border-cyan-900/30 pt-12">
                <div>
                  ...
                  <p className="text-sm text-cyan-700/80 mb-6 max-w-md">
          ...
          {/* TAB 3: PALEO-VALIDATOR (AI Check) */}
          {activeTab === 'paleo' && (
            <motion.div 
              key="paleo"
...
function TabButton({ id, label, icon, active, onClick }) {
  ...
  return (
    <button
      onClick={() => onClick(id)}
      className={`flex items-center gap-2 px-6 py-3 text-xs uppercase tracking-[0.15em] font-bold transition-all border rounded-sm whitespace-nowrap
        ${isActive 
