...
export const metadata = {
  ...
  description: 'Field notes from the waterline of Tethys.',
};

// Revalidate the index every 5 minutes so new WP posts appear without a redeploy
    ...
    <main className="mx-auto max-w-5xl px-6 py-14 md:py-20 text-[#2a1f17]">
      <section className="rounded-3xl border border-amber-900/30 bg-[linear-gradient(145deg,#fbf6ee,#e9dac5)] p-8 shadow-[0_20px_46px_rgba(35,20,8,0.14)] md:p-10">
        ...
        <div className="mt-5 flex flex-wrap gap-2 text-xs uppercase tracking-wide text-[#5b4432]">
          ...
          <span className="rounded-full border border-amber-900/25 bg-[#f8f2e8]/95 px-3 py-1">World of Tethys Lore</span>
        </div>
      </section>
      ...
      <section className="mt-8 grid gap-4">
        {posts.map((post) => (
          ...
          >
            <h2 className="text-xl font-semibold text-[#312115] transition-colors group-hover:text-[#24170f]">
              {post.title}
            </h2>
            {post.excerpt && (
              <p className="mt-2 text-sm leading-relaxed text-[#5a4232] line-clamp-2">{post.excerpt}</p>
            )}
          </Link>
        ))}
      </section>
