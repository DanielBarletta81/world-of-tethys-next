...
type SoundKey = keyof typeof SFX_MANIFEST;
type AudioCache = Record<SoundKey, HTMLAudioElement>;

export default function useSoundFX() {
  const audioCache = useRef<Partial<AudioCache>>({});
  ...
  // 2. The Play Engine
  const play = (key: SoundKey, volume = 0.5, loop = false) => {
