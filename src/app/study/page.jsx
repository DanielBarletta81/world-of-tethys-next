...
export default function StudyPage() {
  ...
  const storiesRead = useMemo(() => {
    const history = playerProfile?.history?.storiesRead;
    return Array.isArray(history) ? history : [];
  }, [playerProfile?.history?.storiesRead]);

  const storyRouteByType = {
    'Field Notes': STORY_MANIFEST['the-sinking-sluice']?.slug,
    'Forbidden Scroll': STORY_MANIFEST['roots-remember']?.slug
  };
      ...
      <div className="relative z-10 pt-28 pb-16 px-6 md:px-12 max-w-7xl mx-auto">
        ...
        <section className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {ARCHIVE_CRATE.map((item) => {
            const storySlug = storyRouteByType[item.type];
            const storyLink = storySlug ? `/study/read/${storySlug}` : item.amazonLink;
            const isRead = storySlug ? storiesRead.includes(storySlug) : false;
            return (
              <ContrabandItem
                key={item.id}
                item={{ ...item, amazonLink: storyLink, read: isRead }}
              />
            );
          })}
        </section>
