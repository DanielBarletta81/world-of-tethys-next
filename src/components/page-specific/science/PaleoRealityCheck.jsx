...
`;
    ...
    try {
      const response = await fetch('/api/paleo/validate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ fantasyConcept })
      });

      const data = await response.json();
      if (!response.ok) {
        throw new Error(data?.error || 'Paleo-uplink failed');
      }
      setAnalysis(data.result);
    } catch (err) {
      console.error('Paleo-uplink failed:', err);
      setError('Paleo-uplink failed. Check server key and quota.');
    } finally {
      setLoading(false);
    }
  };
