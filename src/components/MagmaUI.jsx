...
export const MagmaButton = ({ onClick, children, secondary = false, type = 'button', className = '', ...props }) => {
  if (secondary) {
    return (
      <button 
        onClick={onClick}
        type={type}
        className={`tethys-cta w-full mt-3 px-4 py-2 text-slate-400 bg-slate-900 border border-slate-700 hover:text-white hover:border-slate-500 rounded transition-colors text-sm ${className}`}
        {...props}
      >
        {children}
      </button>
  ...
  return (
    <button
      onClick={onClick}
      type={type}
      className={`tethys-cta w-full px-6 py-3 font-bold text-white transition-all duration-300 rounded bg-gradient-to-r from-orange-600 to-red-700 hover:from-orange-500 hover:to-red-600 shadow-[0_0_15px_rgba(234,88,12,0.5)] hover:shadow-[0_0_25px_rgba(234,88,12,0.8)] ${className}`}
      {...props}
    >
      {children}
    </button>
