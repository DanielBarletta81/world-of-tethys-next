...
function derivePlayerDna(event = {}) {
  ...
  seed = (seed << 56n) | (BigInt(yInt) << 32n) | (BigInt(pm) << 24n) | BigInt(pressureByte);

  const regionFactor = REGION_COEFFICIENTS[event.region] ?? REGION_COEFFICIENTS.default;
  seed = BigInt(Math.abs(Number(seed) * regionFactor)) & 0xffffffffffffffffn;
  ...
  const basePairs = Array.from({ length: 4 }, (_, slot) => ((Number(seed) >> (slot * 8)) & 0b11) % 4);
